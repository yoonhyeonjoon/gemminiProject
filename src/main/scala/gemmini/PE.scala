...
class PE[T <: Data](inputType: T, outputType: T, accType: T,  df: Dataflow.Value, max_simultaneous_matmuls: Int)
  ...
  val last_s = RegEnable(prop, valid) //=(value, boolean)
  val flip = last_s =/= prop
  val shift_offset = Mux(flip, shift, 0.U) //= (cond, true, false)
...
class Launcher[T <: Module](testModule: T, inputType: T, outputType: T, accType: T, max_simultaneous_matmuls: Int)
  extends Module
{
  ...
  val df = Dataflow.BOTH
  val tile = Module(new PE(inputType, outputType, accType, df, max_simultaneous_matmuls))
