...
import gemmini.Util.accumulateTree
...
/**
  ...
  */
class Tile[T <: Data](inputType: T, outputType: T, accType: T,
                      df: Dataflow.Value, tree_reduction: Boolean,
                      max_simultaneous_matmuls: Int, val
                      rows: Int, val columns: Int)(implicit ev: Arithmetic[T]) extends Module {
...
//    df: Dataflow.Value, tree_reduction: Boolean,
//    max_simultaneous_matmuls: Int, val
//  rows: Int, val columns: Int)
  generating(new Tile(inputType = UInt(8.W), outputType = UInt(4.W), accType = UInt(5.W), df =  Dataflow.BOTH, max_simultaneous_matmuls = 3, tree_reduction = true, rows = 5, columns = 5), dir = "generated/gemmini")
}
